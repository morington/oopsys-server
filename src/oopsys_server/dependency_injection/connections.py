...
def before_cursor_execute(conn: Connection, cursor: Any, statement: str, parameters: Any, context: Any, executemany: bool) -> None:
    conn.info["query_start_time"] = time.perf_counter()

def set_query_log(duration: float) -> None:
    logger.debug("SQL Query complete", duration=round(duration, 6))

def after_cursor_execute(conn: Connection, cursor: Any, statement: str, parameters: Any, context: Any, executemany: bool) -> None:
...
async def _create_engine(url: str | URL, ssl: str) -> AsyncEngine | None:
    engine = create_async_engine(url, pool_size=10, max_overflow=5, pool_recycle=300, pool_pre_ping=True, pool_timeout=30, connect_args={"ssl": ssl})
