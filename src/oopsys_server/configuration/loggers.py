...
class Loggers(InitLoggers):
    ...
    ingest = LoggerReg(name="INGEST", level=LoggerReg.Level.INFO)
    auth = LoggerReg(name="AUTH", level=LoggerReg.Level.INFO)
    notifier = LoggerReg(name="NOTIFIER", level=LoggerReg.Level.INFO)
    liveness = LoggerReg(name="LIVENESS", level=LoggerReg.Level.INFO)
    web = LoggerReg(name="WEB", level=LoggerReg.Level.INFO)
