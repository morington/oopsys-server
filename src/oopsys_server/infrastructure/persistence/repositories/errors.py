...
class ErrorRepository:
    ...
    async def get_or_create_group(self, *, agent_id: str, fingerprint: str, service: str, environment: str, exception_type: str, title: str, severity: Severity) -> tuple[ErrorGroup, bool]:
        result = await self._session.execute(select(ErrorGroup).where(ErrorGroup.agent_id == agent_id, ErrorGroup.fingerprint == fingerprint))
        group = result.scalar_one_or_none()
        if group is not None:
            return (group, False)
        group = ErrorGroup(agent_id=agent_id, fingerprint=fingerprint, service=service, environment=environment, exception_type=exception_type, title=title, severity=severity, count=0)
    ...
    async def list_groups(self, *, agent_ids: list[str], limit: int=200) -> list[ErrorGroup]:
        if not agent_ids:
            return []
        result = await self._session.execute(select(ErrorGroup).where(ErrorGroup.agent_id.in_(agent_ids)).order_by(ErrorGroup.last_seen.desc()).limit(limit))
        return list(result.scalars().all())

    async def recent_reports(self, group_id: uuid.UUID, limit: int=20) -> list[ErrorReport]:
        result = await self._session.execute(select(ErrorReport).where(ErrorReport.group_id == group_id).order_by(ErrorReport.occurred_at.desc()).limit(limit))
        return list(result.scalars().all())
...
class AgentFaultRepository:
    ...
    async def list_for_agents(self, agent_ids: list[str], limit: int=100) -> list[AgentFaultRecord]:
        if not agent_ids:
            return []
        result = await self._session.execute(select(AgentFaultRecord).where(AgentFaultRecord.agent_id.in_(agent_ids)).order_by(AgentFaultRecord.occurred_at.desc()).limit(limit))
        return list(result.scalars().all())
...
class SelfErrorRepository:
    ...
    async def record(self, *, component: str, exception_type: str, message: str, traceback: str, fingerprint: str, context: dict | None=None) -> SelfError:
        ...
        if existing is not None:
            ...
            return existing
        record = SelfError(component=component, exception_type=exception_type, message=message, traceback=traceback, fingerprint=fingerprint, context=context or {})
