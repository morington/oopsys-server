...
class MetricsRepository:
    ...
    async def latest(self, agent_id: str) -> ServerMetricRecord | None:
        result = await self._session.execute(
            select(ServerMetricRecord)
            .where(ServerMetricRecord.agent_id == agent_id)
            .order_by(ServerMetricRecord.captured_at.desc())
            .limit(1)
        )
    ...
    async def history(self, agent_id: str, *, since_minutes: int = 60) -> list[ServerMetricRecord]:
        ...
        result = await self._session.execute(
            select(ServerMetricRecord)
            .where(ServerMetricRecord.agent_id == agent_id, ServerMetricRecord.captured_at >= cutoff)
            .order_by(ServerMetricRecord.captured_at)
        )
...
class ContainerRepository:
    ...
    async def upsert(self, agent_id: str, payload: ContainerStatePayload) -> ContainerStateRecord:
        result = await self._session.execute(
            select(ContainerStateRecord).where(
                ContainerStateRecord.agent_id == agent_id, ContainerStateRecord.container_id == payload.container_id
            )
        )
    ...
    async def list_for_agents(self, agent_ids: list[str]) -> list[ContainerStateRecord]:
        if not agent_ids:
            return []
        result = await self._session.execute(
            select(ContainerStateRecord)
            .where(ContainerStateRecord.agent_id.in_(agent_ids))
            .order_by(ContainerStateRecord.name)
        )
        return list(result.scalars().all())
    ...
    async def assign_project(self, agent_id: str, container_id: str, project_id: uuid.UUID | None) -> None:
        result = await self._session.execute(
            select(ContainerStateRecord).where(
                ContainerStateRecord.agent_id == agent_id, ContainerStateRecord.container_id == container_id
            )
        )
        record = result.scalar_one_or_none()
    ...
    async def set_hidden(self, agent_id: str, container_id: str, hidden: bool) -> None:
        result = await self._session.execute(
            select(ContainerStateRecord).where(
                ContainerStateRecord.agent_id == agent_id, ContainerStateRecord.container_id == container_id
            )
