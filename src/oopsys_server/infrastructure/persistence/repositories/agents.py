...
class AgentTokenRepository:
    ...
    async def is_bound_to_account(self, token_id: uuid.UUID) -> bool:
        result = await self._session.execute(
            select(AccountAgent.id).where(AccountAgent.agent_token_id == token_id).limit(1)
        )
    ...
    async def accounts_with_labels_for_agent(self, agent_id: str) -> list[tuple[Account, str | None]]:
        result = await self._session.execute(
            select(Account, AgentToken.label)
            .join(AccountAgent, AccountAgent.account_id == Account.id)
            .join(AgentToken, AgentToken.id == AccountAgent.agent_token_id)
            .where(AgentToken.agent_id == agent_id)
        )
        return [(account, label) for account, label in result.unique().all()]
...
class AgentRepository:
    ...
    async def list_stale(self, stale_seconds: int) -> list[Agent]:
        ...
        result = await self._session.execute(
            select(Agent).where(Agent.last_seen < cutoff, Agent.status == AgentStatus.ONLINE)
        )
        return list(result.scalars().all())
