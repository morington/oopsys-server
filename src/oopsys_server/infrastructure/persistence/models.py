...
class ErrorGroup(Base):
    ...
    __table_args__ = (
        UniqueConstraint("agent_id", "fingerprint", name="uq_group_agent_fingerprint"),
        Index("ix_group_agent_lastseen", "agent_id", "last_seen"),
    )
    id: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, default=_uuid)
...
class ContainerStateRecord(Base):
    ...
    __table_args__ = (
        UniqueConstraint("agent_id", "container_id", name="uq_container_agent_id"),
        Index("ix_container_agent", "agent_id"),
    )
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=True)
    agent_id: Mapped[str] = mapped_column(String(36), index=True)
