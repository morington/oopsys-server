...
PAGES: dict[str, str] = {
    ...
    "server_detail": "server_detail.html",
    "errors": "errors.html",
    "error_detail": "error_detail.html",
...
def _agent(name: str, status: AgentStatus = AgentStatus.ONLINE) -> SimpleNamespace:
    return _ns(
        ...
        last_seen=_NOW - timedelta(seconds=12 if status is AgentStatus.ONLINE else 600),
    )
...
def _group(
    exc: str, service: str, severity: Severity, count: int, status: ErrorGroupStatus = ErrorGroupStatus.OPEN
) -> SimpleNamespace:
    return _ns(
        id=uuid.uuid4(),
        agent_id=str(uuid.uuid4()),
        ...
        exception_type=exc,
        title=f"{exc}: something went wrong in {service}",
        severity=severity,
        status=status,
        count=count,
        first_seen=_NOW - timedelta(hours=4),
...
def _dashboard(scenario: str) -> dict[str, Any]:
    ...
    return {
        "agents_total": 2,
        "agents_online": 1,
        "agents_down": 1 if scenario == "down" else 0,
        "open_errors": 3,
        "server_cards": [
            {"agent": online, "latest": _metric(72 if scenario == "critical" else 35, 58)},
            {"agent": down, "latest": _metric(5, 20)},
        ],
        "notifications": [
            _notification("Агент недоступен: worker-eu", Severity.CRITICAL, "Нет данных более 90 с"),
            _notification("ValueError в cryptobot", severity, "cryptobot · production"),
        ],
        "groups": [
            _group("ValueError", "cryptobot", severity, 42 if scenario == "spam" else 3),
            _group("TimeoutError", "payments", Severity.ERROR, 7),
        ],
    }
...
def _agents(scenario: str) -> dict[str, Any]:
    ...
    return {
        "items": [
            {"token": _ns(id=uuid.uuid4(), label="prod-1", agent_id=a1.agent_id, is_active=True), "agent": a1},
            {"token": _ns(id=uuid.uuid4(), label="eu-worker", agent_id=a2.agent_id, is_active=True), "agent": a2},
            {"token": _ns(id=uuid.uuid4(), label="new", agent_id=None, is_active=True), "agent": None},
        ]
    }
...
def _servers(scenario: str) -> dict[str, Any]:
    ...
    return {
        "cards": [
            {"agent": _agent("web-prod"), "latest": _metric(cpu, 61)},
            {"agent": _agent("worker-eu", AgentStatus.DOWN), "latest": _metric(4, 18)},
        ]
    }
...
def _server_detail(scenario: str) -> dict[str, Any]:
    ...
    cfg = {
        ...
        "data": {
            ...
            "datasets": [
                {
                    "label": "CPU %",
                    "data": [20, 35, 60, 88, 70, 40, 30, 33, 41, 55, 62, 48],
                    "borderColor": "#3b6cf6",
                    "tension": 0.3,
                    "pointRadius": 0,
                },
                {
                    "label": "RAM %",
                    "data": [40, 42, 45, 60, 58, 55, 50, 52, 53, 57, 61, 59],
                    "borderColor": "#1f9d57",
                    "tension": 0.3,
                    "pointRadius": 0,
                },
    ...
    return {
        "agent": _agent("web-prod"),
        "latest": _metric(88 if scenario == "critical" else 34, 61),
        "chart_config": cfg,
        "has_history": scenario != "empty",
        "containers": [_container("web", "nginx:1.27"), _container("db", "postgres:16", "exited")],
    }
...
def _errors(scenario: str) -> dict[str, Any]:
    ...
    return {
        "groups": [
            _group("ValueError", "cryptobot", sev, 42 if scenario == "spam" else 3),
            _group("TimeoutError", "payments", Severity.ERROR, 7),
            _group("KeyError", "api", Severity.ERROR, 1, ErrorGroupStatus.RESOLVED),
        ]
    }
...
def _error_detail(scenario: str) -> dict[str, Any]:
    ...
    reports = [
        _ns(
            ...
            occurred_at=_NOW - timedelta(minutes=i * 3),
            context={"user_id": 1024, "endpoint": "/pay"},
...
def _projects(scenario: str) -> dict[str, Any]:
    ...
    return {
        ...
        "rules_by_project": {
            pid: [
                _ns(match_type="service", match_value="cryptobot"),
                _ns(match_type="label", match_value="com.docker.compose.project=cryptobot"),
            ]
        },
    }
...
def _system(scenario: str) -> dict[str, Any]:
    ...
    return {
        "nats_connected": scenario != "down",
        "self_errors": [
            _ns(
                component="web:/agents/ingest",
        ...
        "agent_faults": [
            _ns(
                ...
                exception_type="DockerException",
                occurred_at=_NOW,
            )
        ],
    }
...
def _settings(scenario: str) -> dict[str, Any]:
    return {
        "error": "Пароли не совпадают" if scenario == "critical" else None,
        "ok": "Настройки сохранены" if scenario == "default" else None,
    }
