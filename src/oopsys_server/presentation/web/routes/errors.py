...
@router.get("/errors")
async def errors_page(request: Request, tokens: FromDishka[AgentTokenRepository], errors: FromDishka[ErrorRepository], account: Account=Depends(require_account)) -> Response:
...
async def error_detail(request: Request, group_id: uuid.UUID, tokens: FromDishka[AgentTokenRepository], errors: FromDishka[ErrorRepository], account: Account=Depends(require_account)) -> Response:
...
async def _set_status(group_id: uuid.UUID, status: ErrorGroupStatus, tokens: AgentTokenRepository, errors: ErrorRepository, session: AsyncSession, account: Account) -> None:
    group = await errors.get_group(group_id)
    if group is None or group.agent_id not in await _agent_ids(tokens, account):
        raise HTTPException(status_code=404)
...
async def mute(request: Request, group_id: uuid.UUID, tokens: FromDishka[AgentTokenRepository], errors: FromDishka[ErrorRepository], session: FromDishka[AsyncSession], account: Account=Depends(require_account)) -> Response:
...
async def resolve(request: Request, group_id: uuid.UUID, tokens: FromDishka[AgentTokenRepository], errors: FromDishka[ErrorRepository], session: FromDishka[AsyncSession], account: Account=Depends(require_account)) -> Response:
    await _set_status(group_id, ErrorGroupStatus.RESOLVED, tokens, errors, session, account)
    return RedirectResponse(f"/errors/{group_id}", status_code=303)

@router.post("/errors/{group_id}/reopen")
async def reopen(request: Request, group_id: uuid.UUID, tokens: FromDishka[AgentTokenRepository], errors: FromDishka[ErrorRepository], session: FromDishka[AsyncSession], account: Account=Depends(require_account)) -> Response:
